...
import { Camera, Upload, ArrowLeft, ArrowRight, Check, X, Award } from 'lucide-react';
...
export default function BecomeProvider() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Update user with CNIC and add provider role
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      <div className="bg-white dark:bg-gray-900 shadow-sm p-4 border-b border-gray-200 dark:border-gray-800">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <h1 className="text-xl font-bold text-blue-600 dark:text-blue-400">Become a Provider</h1>
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full p-6">
        ...
        {currentStep === 0 && (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">CNIC Number *</label>
              <input type="text" value={formData.cnicNumber}
                onChange={(e) => { const formatted = formatCNIC(e.target.value); setFormData(prev => ({ ...prev, cnicNumber: formatted })); setErrors(prev => ({ ...prev, cnicNumber: undefined })); }}
                maxLength={15} className={`w-full px-4 py-3 rounded-lg border bg-white dark:bg-gray-800 text-gray-900 dark:text-white ${errors.cnicNumber ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'} focus:ring-2 focus:ring-blue-500`}
                placeholder="00000-0000000-0" />
              <div className="text-red-500 text-sm mt-1 h-5">{errors.cnicNumber}</div>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">CNIC Front *</label>
              {formData.cnicFrontPreview ? (
                <div className="relative"><img src={formData.cnicFrontPreview} alt="CNIC Front" className="w-full max-w-md rounded-lg" />
                  <button type="button" onClick={() => setFormData(prev => ({ ...prev, cnicFrontFile: null, cnicFrontPreview: null }))} className="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full"><X size={16} /></button>
                </div>
              ) : (
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">CNIC Back *</label>
              {formData.cnicBackPreview ? (
                <div className="relative"><img src={formData.cnicBackPreview} alt="CNIC Back" className="w-full max-w-md rounded-lg" />
                  <button type="button" onClick={() => setFormData(prev => ({ ...prev, cnicBackFile: null, cnicBackPreview: null }))} className="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full"><X size={16} /></button>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-3">
        ...
        {currentStep === 1 && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Select Your Services</h2>
            <p className="text-gray-600 dark:text-gray-400">You can select multiple services</p>
            <div className="grid grid-cols-2 gap-3">
              {SERVICES.map(service => (
                <button key={service.id} type="button" onClick={() => toggleService(service.id)}
                  className={`p-4 rounded-xl border-2 text-center transition-all ${selectedServices.includes(service.id) ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/30 ring-2 ring-blue-600' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                  <div className="text-2xl mb-1">{service.icon}</div>
                  <div className="font-medium text-sm text-gray-900 dark:text-white">{service.name}</div>
                </button>
              ))}
            </div>
            <div className="text-red-500 text-sm h-5">{errors.services}</div>

            {/* Vehicle type if ride selected */}
            {selectedServices.includes('ride') && (
              <div className="space-y-3 pt-2">
                <h3 className="font-semibold text-gray-900 dark:text-white">Select Vehicle Type</h3>
                <div className="grid grid-cols-3 gap-2">
                  {['bike', 'rickshaw', 'car'].map(v => (
                    <button key={v} type="button" onClick={() => setVehicleType(v)}
                      className={`p-3 rounded-xl border-2 text-center capitalize transition-all ${vehicleType === v ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/30 ring-2 ring-blue-600' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'}`}>
                      {v === 'bike' ? '🏍️' : v === 'rickshaw' ? '🛺' : '🚗'} {v}
                    </button>
                  ))}
                </div>
              </div>
        ...
        {currentStep === 2 && (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Certificate / License</label>
              {formData.certificatePreview ? (
                <div className="relative"><img src={formData.certificatePreview} alt="Certificate" className="w-full max-w-md rounded-lg" />
                  <button type="button" onClick={() => setFormData(prev => ({ ...prev, certificateFile: null, certificatePreview: null }))} className="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full"><X size={16} /></button>
                </div>
              ) : (
                <button type="button" onClick={() => triggerFileUpload('certificate', false)} className="flex flex-col items-center gap-2 p-6 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl hover:border-blue-500 w-full">
                  <Award size={32} className="text-gray-400 dark:text-gray-500" /><span className="text-sm text-gray-600 dark:text-gray-400">Upload Certificate (PDF/Image)</span>
                </button>
              )}
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {['< 1 year', '1 year', '2 years', '3 years', '4 years', '5+ years'].map(exp => (
                  <button key={exp} type="button" onClick={() => setFormData(prev => ({ ...prev, experience: exp }))}
                    className={`py-2.5 rounded-xl text-sm font-medium transition-all ${formData.experience === exp ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>
                    {exp}
                  </button>
                ))}
              </div>
        ...
        {currentStep === 3 && (
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 space-y-2 border border-gray-200 dark:border-gray-700">
              ...
              <ReviewRow label="Services" value={selectedServices.map(s => SERVICES.find(x => x.id === s)?.name).join(', ')} />
              {selectedServices.includes('ride') && <ReviewRow label="Vehicle" value={vehicleType || 'Not selected'} />}
              {formData.cnicFrontPreview && <div><span className="text-sm text-gray-500">CNIC Front:</span><img src={formData.cnicFrontPreview} alt="Front" className="w-32 rounded mt-1" /></div>}
              {formData.cnicBackPreview && <div><span className="text-sm text-gray-500">CNIC Back:</span><img src={formData.cnicBackPreview} alt="Back" className="w-32 rounded mt-1" /></div>}
              {formData.certificatePreview && <div><span className="text-sm text-gray-500">Certificate:</span><img src={formData.certificatePreview} alt="Cert" className="w-32 rounded mt-1" /></div>}
            </div>
        ...
        <div className="flex justify-between mt-8">
          {currentStep > 0 && (
            <button type="button" onClick={() => setCurrentStep(prev => prev - 1)} className="flex items-center gap-2 px-4 py-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
              <ArrowLeft size={20} /> Back
            </button>
          )}
          <div className="flex-1" />
          {currentStep < 3 && (
            <button type="button" onClick={() => { if (validateStep(currentStep)) setCurrentStep(prev => prev + 1) }}
