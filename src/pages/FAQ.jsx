...
export default function FAQ() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900" dir={lang === 'ur' ? 'rtl' : 'ltr'}>
      <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 h-16 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3">
          <Link to="/" className="text-gray-500"><ArrowLeft className="w-5 h-5" /></Link>
          <h1 className="text-lg font-semibold text-gray-900 dark:text-white">FAQ</h1>
        </div>
        <div className="flex items-center gap-1 bg-purple-50 dark:bg-gray-700 p-1 rounded-lg">
      ...
      <div className="max-w-2xl mx-auto p-4">
        <div className="relative mb-6">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t('Search FAQs...', 'سوالات تلاش کریں...')} className="w-full pl-10 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-purple-500/50 dark:text-white" />
        </div>
          ...
          {filtered.map((f, i) => (
            <div key={i} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
              <button onClick={() => setActive(active === i ? null : i)} className="w-full flex items-center justify-between px-5 py-4 text-left">
                ...
                <ChevronRight className={`w-4 h-4 text-gray-400 transition-transform ${active === i ? 'rotate-90' : ''}`} />
              </button>
