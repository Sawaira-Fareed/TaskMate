...
import { ArrowLeft, Send, Check, CheckCheck, AlertTriangle, WifiOff } from 'lucide-react'
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        setCurrentUser(user)
        const { data: bkg } = await supabase.from('bookings').select('*, customer:customer_id(full_name), provider:provider_id(user_id, user:user_id(full_name))').eq('id', bookingId).single()
