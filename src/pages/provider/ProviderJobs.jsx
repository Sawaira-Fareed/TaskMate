...
import { ArrowLeft, Calendar, CheckCircle, Clock, XCircle, Star, MessageCircle, User, DollarSign, Wrench, Plug, ShoppingBag, Monitor } from 'lucide-react'
      ...
      <div className="max-w-2xl mx-auto p-4">
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-3 text-center border border-gray-100 dark:border-gray-700">
            ...
            <p className="text-[10px] text-gray-500">{t('Incoming', 'آنے والے')}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-3 text-center border border-gray-100 dark:border-gray-700">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-3 text-center border border-gray-100 dark:border-gray-700">
            <p className="text-lg font-bold text-amber-600">PKR {stats.earnings}</p>
            <p className="text-[10px] text-gray-500">{t('Earned', 'کمایا')}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-3 text-center border border-gray-100 dark:border-gray-700">
            <p className="text-lg font-bold text-gray-900 dark:text-white">{stats.total}</p>
            <p className="text-[10px] text-gray-500">{t('Total', 'کل')}</p>
          </div>
        </div>
        ...
        <div className="flex gap-1 mb-4 bg-white dark:bg-gray-800 rounded-xl p-1 border border-gray-100 dark:border-gray-700">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 py-2.5 text-xs font-medium rounded-lg transition-all flex items-center justify-center gap-1.5 ${activeTab === tab.id ? 'bg-purple-600 text-white shadow-md' : 'text-gray-500 dark:text-gray-400'}`}>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-gray-500">{t('No jobs found', 'کوئی کام نہیں ملا')}</p>
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map(j => {
              const Icon = serviceIcons[j.service_type] || Wrench
              return (
                <div key={j.id} className="bg-gradient-to-br from-purple-600 to-purple-800 rounded-2xl shadow-lg shadow-purple-500/20 p-4 text-white">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm font-semibold capitalize">{j.service_type}</p>
                        {j.customer?.full_name && (
                          <p className="text-xs text-white/60">{j.customer.full_name}</p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    {j.rating && (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs font-medium">{j.rating}/5</span>
                      </div>
                    )}
                    {j.provider_earnings && (
