...
import { Plus, Clock, CheckCircle, Star, ChevronRight, LogOut, Home, ClipboardList, Calendar, Bell, User, Settings } from 'lucide-react'
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-4 lg:p-6 space-y-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <button onClick={() => navigate('/customer/create-request')} className="bg-purple-600 hover:bg-purple-700 text-white rounded-2xl p-5 transition-all hover:shadow-lg hover:shadow-purple-200/30 flex items-center gap-4 text-left">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => navigate('/customer/my-requests')} className="text-sm text-purple-600 hover:text-purple-700 font-medium flex items-center gap-1">{t('View All', 'سب دیکھیں')} <ChevronRight className="w-4 h-4" /></button>
            </div>
            {activeRequests.length === 0 ? (
              <div className="text-center py-8">
                <Clock className="w-10 h-10 text-gray-300 mx-auto mb-2" />
                <p className="text-sm text-gray-500">{t('No active requests', 'کوئی فعال درخواست نہیں')}</p>
                <button onClick={() => navigate('/customer/create-request')} className="text-sm text-purple-600 font-medium mt-1 inline-block">{t('Create one now', 'ابھی بنائیں')}</button>
              </div>
            ) : (
              <div className="space-y-2">
                {activeRequests.map(req => (
                  <button key={req.id} onClick={() => navigate(`/customer/request/${req.id}`)} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-colors text-left">
                    <div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => navigate('/customer/bookings')} className="text-sm text-purple-600 hover:text-purple-700 font-medium flex items-center gap-1">{t('View All', 'سب دیکھیں')} <ChevronRight className="w-4 h-4" /></button>
            </div>
