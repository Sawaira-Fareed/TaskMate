  ...
  useEffect(() => {
    async function init() {
      ...
      const { data: bkg } = await supabase
        .from('bookings')
        .select('*, provider:provider_id(user_id, user:user_id(full_name)), customer:customer_id(full_name)')
        .eq('id', bookingId)
        .single()
      setBooking(bkg)
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-sm text-gray-500">{t('No messages yet. Say hello!', 'ابھی تک کوئی پیغام نہیں۔ ہیلو کہیں!')}</p>
          </div>
        ) : (
          messages.map(msg => {
                  ...
                  <div className={`flex items-center gap-1 mt-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                    ...
                    {status && (
                      <span className={`text-[10px] ${status.color}`}>
                        {status.icon === '✓✓' ? <CheckCheck className="w-3 h-3 inline" /> : <Check className="w-3 h-3 inline" />}
                      </span>
                    )}
                  </div>
                </div>
      ...
      <div className="bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 p-4">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-2.5 text-sm bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 outline-none focus:ring-2 focus:ring-purple-500"
          />
          <button
            onClick={handleSend}
            disabled={!text.trim() || sending}
            className="w-10 h-10 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-300 dark:disabled:bg-gray-700 text-white rounded-xl flex items-center justify-center transition-colors"
          >
