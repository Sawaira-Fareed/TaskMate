...
import { ArrowLeft, ArrowRight, Check, Clock, AlertTriangle, Zap, Wrench, Plug, ShoppingBag, Monitor, MapPin, Calendar, Mic, Send, Square, ChevronRight } from 'lucide-react'
...
const [coordinates, setCoordinates] = useState(null)
  ...
  // Sub-steps for Details step
  const [detailStep, setDetailStep] = useState(1) // 1=description, 2=date, 3=time
...
const shareLocationLink = () => {
  ...
  const canProceed = () => {
    ...
    if (step === 3) {
      setValidationError('')
      if (!description && !voiceBlob) { setValidationError(t('Description or voice note is required', 'تفصیل یا وائس نوٹ درکار ہے')); return false }
      if (!date) { setValidationError(t('Date is required', 'تاریخ درکار ہے')); return false }
      if (!time) { setValidationError(t('Time is required', 'وقت درکار ہے')); return false }
      if (!location) { setValidationError(t('Location is required', 'مقام درکار ہے')); return false }
      return true
    }
    return true
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (voiceBlob) {
        ...
        const { error: uploadError } = await supabase.storage.from('requests').upload(fileName, voiceBlob)
        if (!uploadError) {
      ...
      try {
        ...
        parsedIntent = { service_type: serviceType, preferred_date: date, preferred_time: time, language: lang, urgency: priority, items: [] }
      }
  ...
  description
      ...
      matchProviders(parsedIntent, user.id)
        .then(providers => { if (providers?.length > 0) contactProviderLoop(request.id, providers) })
  ...
  const getNext7Days = () => {
    return [...Array(7)].map((_, i) => {
      ...
      const isToday = i === 0
      const dayName = isToday ? t('Today', 'آج') : d.toLocaleDateString(lang === 'ur' ? 'ur-PK' : 'en-US', { weekday: 'long' })
      const dateDisplay = d.toLocaleDateString(lang === 'ur' ? 'ur-PK' : 'en-US', { day: 'numeric', month: 'long' })
      return { dateStr, dayName, dateDisplay, isToday }
    })
  }

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-start justify-center p-4 pt-20 page-enter" dir={lang === 'ur' ? 'rtl' : 'ltr'}>
      <div className="w-full max-w-lg">
          ...
          <div className="flex items-center justify-center gap-1 mt-3">
            {STEPS.map((s, i) => (
              <div key={s} className="flex items-center gap-1">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium transition-all ${i + 1 <= step ? 'bg-purple-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>
                  {i + 1 < step ? <Check className="w-3 h-3" /> : i + 1}
                </div>
                {i < STEPS.length - 1 && <div className={`w-6 h-0.5 transition-all ${i + 1 < step ? 'bg-purple-600' : 'bg-gray-200 dark:bg-gray-700'}`} />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          ...
          {step === 1 && (
            <div className="space-y-3">
              ...
              {priorities.map(p => {
                const Icon = p.icon
                return (
                  <button key={p.id} onClick={() => setPriority(p.id)} className={`w-full p-4 rounded-xl border-2 text-left transition-all duration-200 ${priority === p.id ? `${p.border} ${p.bg}` : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
          ...
          {step === 2 && (
            <div className="grid grid-cols-2 gap-3">
              {services.map(s => {
                const Icon = s.icon
                return (
                  <button key={s.id} onClick={() => setServiceType(s.id)} className={`p-4 rounded-xl border-2 text-center transition-all duration-200 hover:shadow-md ${serviceType === s.id ? 'border-purple-600 bg-purple-50 dark:bg-purple-900/30 shadow-md' : 'border-gray-200 dark:border-gray-700 hover:border-purple-300 dark:hover:border-purple-700'}`}>
          ...
          {step === 3 && (
            <div>
              {/* Detail sub-steps indicator */}
              <div className="flex items-center gap-2 mb-4">
                {[1,2,3].map(s => (
                  <div key={s} className={`flex-1 h-1 rounded-full transition-all ${detailStep >= s ? 'bg-purple-600' : 'bg-gray-200 dark:bg-gray-700'}`} />
                ))}
              </div>

              {/* Detail Step 1: Description */}
              {detailStep === 1 && (
                <div className="space-y-4">
                  ...
                  <textarea
                    ...
                    placeholder={voiceNote ? t('Add text notes (optional)...', 'مزید تفصیل لکھیں (اختیاری)...') : t('Describe what you need...', 'تفصیل سے لکھیں کہ آپ کو کیا چاہیے...')}
                    className="w-full border border-gray-200 dark:border-gray-600 rounded-xl px-3.5 py-2.5 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:border-purple-500 focus:ring-2 focus:ring-purple-100 dark:focus:ring-purple-900 outline-none resize-none placeholder-gray-400 dark:placeholder-gray-500"
                    autoFocus
                  />
    ...
    {coordinates && (
      <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-1 flex items-center gap-1">
        <MapPin className="w-3 h-3" /> {coordinates.lat.toFixed(6)}, {coordinates.lng.toFixed(6)}
              ...
              {detailStep === 2 && (
                <div>
                  <button onClick={() => setDetailStep(1)} className="text-sm text-purple-600 dark:text-purple-400 mb-3 flex items-center gap-1">
                    <ArrowLeft className="w-3.5 h-3.5" /> {t('Back', 'واپس')}
                  </button>
                  <div className="space-y-2">
                    {getNext7Days().map(d => (
                      <button key={d.dateStr} onClick={() => { setDate(d.dateStr); setDetailStep(3) }}
                        className="w-full flex items-center justify-between p-3.5 rounded-xl bg-gray-50 dark:bg-gray-700 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-all duration-200 border border-transparent hover:border-purple-200 dark:hover:border-purple-700">
                        <div className="text-left"><p className="text-sm font-medium text-gray-900 dark:text-white">{d.dayName}</p><p className="text-xs text-gray-500 dark:text-gray-400">{d.dateDisplay}</p></div>
              ...
              {detailStep === 3 && (
                <div>
                  <button onClick={() => setDetailStep(2)} className="text-sm text-purple-600 dark:text-purple-400 mb-3 flex items-center gap-1">
                    <ArrowLeft className="w-3.5 h-3.5" /> {t('Back', 'واپس')}
                  </button>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                    {date && new Date(date).toLocaleDateString(lang === 'ur' ? 'ur-PK' : 'en-US', { weekday: 'long', day: 'numeric', month: 'long' })}
                  </p>
                  ...
                  <div className="flex items-center gap-2 justify-center mb-6">
                    <input
                      ...
                      value={(() => { if (!time) return ''; const h = parseInt(time.split(':')[0]); if (isNaN(h)) return ''; if (h === 0) return '12'; if (h > 12) return String(h - 12); return String(h) })()}
                      onChange={(e) => {
                        const raw = e.target.value.replace(/\D/g, '')
                        if (raw === '') { setTime(''); return }
                        let val = parseInt(raw); if (val > 12) val = 12; if (val < 1) val = 1
                        const ampm = (time && time.includes('PM')) ? 'PM' : 'AM'
                        const realHour = ampm === 'PM' ? (val === 12 ? 12 : val + 12) : (val === 12 ? 0 : val)
                        setTime(`${String(realHour).padStart(2, '0')}:00`)
                      }}
                    ...
                    <button type="button"
                      onClick={() => {
                        if (!time) { setTime('08:00'); return }
                        const hour = parseInt(time.split(':')[0])
                        const isPM = time.includes('PM') || hour >= 12
                        const newHour = isPM ? (hour >= 12 ? hour - 12 : hour) : hour + 12
                        setTime(`${String(newHour).padStart(2, '0')}:00`)
                      }}
                      className={`px-4 py-3 rounded-xl text-sm font-bold min-w-[56px] transition-all ${time && (time.includes('PM') || parseInt(time) >= 12) ? 'bg-indigo-600 text-white' : 'bg-amber-500 text-white'}`}
                    >
                      {time && (time.includes('PM') || parseInt(time) >= 12) ? 'PM' : 'AM'}
                    </button>
                  </div>
                  ...
                  <div className="flex gap-2 text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 rounded-xl p-3 mb-4">
                    ...
                    <span>{new Date(date).toLocaleDateString(lang === 'ur' ? 'ur-PK' : 'en-US', { weekday: 'short', day: 'numeric', month: 'short' })}</span>
                    <span>•</span>
                    <span className="text-xs text-gray-500 truncate">{description}</span>
                  </div>

                  <button onClick={() => { if (time) { setDetailStep(1); setStep(4); setValidationError('') } else setValidationError(t('Time is required', 'وقت درکار ہے')) }} className="w-full py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-sm font-medium transition-all">
                    {t('Continue to Review', 'جائزہ پر جائیں')}
                  </button>
                </div>
              )}
            </div>
          ...
          {step === 4 && (
            <div className="space-y-3">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">{t('Time', 'وقت')}</span><span className="font-medium text-gray-900 dark:text-white">{time}</span></div>
          ...
          <div className="flex gap-2 mt-6">
            {step > 1 && (
              <button onClick={() => setStep(step - 1)} className="flex-1 flex items-center justify-center gap-1.5 border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-2.5 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all">
                <ArrowLeft className="w-3.5 h-3.5" /> {t('Back', 'واپس')}
              </button>
            )}
            {step < 3 ? (
              <button onClick={() => { if (canProceed()) setStep(step + 1) }} className="flex-1 bg-purple-600 hover:bg-purple-700 text-white rounded-xl px-4 py-2.5 text-sm font-medium flex items-center justify-center gap-1.5 transition-all hover:shadow-lg">
                {t('Next', 'اگلا')} <ArrowRight className="w-3.5 h-3.5" />
              </button>
            ) : step === 3 ? (
              <button onClick={() => { if (canProceed()) setStep(4) }} className="flex-1 bg-purple-600 hover:bg-purple-700 text-white rounded-xl px-4 py-2.5 text-sm font-medium flex items-center justify-center gap-1.5 transition-all hover:shadow-lg">
                {t('Review', 'جائزہ')} <ArrowRight className="w-3.5 h-3.5" />
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={loading} className="flex-1 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-400 text-white rounded-xl px-4 py-2.5 text-sm font-medium flex items-center justify-center gap-1.5 transition-all hover:shadow-lg">
